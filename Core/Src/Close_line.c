...
/*
 * 左右轮速度查表，每一项为：{ 左轮速度, 右轮速度 }，速度范围是 TB6612 的 0~100。
 * 差速按位置线性分配：外侧轮保持 40 不变，内侧轮随偏离程度以固定步长 4 线性减小，
 * 相邻档位差速恒定，避免原表差速骤变（3 -> 18 -> 28）导致的转向过猛和抖动。
 * 想要转向更缓：把 24 改大（步长变小）；想要转向更灵：把 24 改小（步长变大）。
 */
static const uint8_t CLOSE_LINE_SPEED_TABLE[][2] = {
    {24, 40}, /* 只有最左侧传感器压线：左轮最小，差速最大 */

    {28, 40}, /* 最左侧和左侧第二路同时压线 */
    {32, 40}, /* 只有左侧第二路压线 */

    {36, 40}, /* 左侧第二路和中间传感器同时压线 */


    {40, 40}, /* 只有中间传感器压线，直线行驶 */


    {40, 36}, /* 中间传感器和偏右侧传感器同时压线 */

    {40, 32}, /* 只有偏右侧传感器压线 */
    {40, 28}, /* 偏右侧和最右侧传感器同时压线 */

    {40, 24}, /* 只有最右侧传感器压线：右轮最小，差速最大 */
};
